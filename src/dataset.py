...
class MedicalRegistrationDataset(Dataset):
    ...
    def __init__(
        ...
        for _, row in df.iterrows():
            ...
            ct_mask_path = str(out_dir / f"{sid}_ct_mask.npy")  # CT brain mask for eval
            ...
            ct_mask_ok   = Path(ct_mask_path).exists()
            ...
            self.samples.append({
                ...
                "ct_mask_path": ct_mask_path if ct_mask_ok else None,  # may be None if not yet generated
            })
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        # Load CT mask only when available (generated by scripts/generate_ct_mask_npy.py)
        if info.get("ct_mask_path"):
            sample["ct_mask"] = self._load_npy(info["ct_mask_path"], normalize=False)
